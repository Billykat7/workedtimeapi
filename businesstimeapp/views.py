...
def homePage(request):
	"""
	Welcome to the home page of our worked-time calculating API.
	Acceptable requests are during Weekdays, between 08:00:00 & 17:00:00.
	No ZA public holidays.
	"""
	return Response({
...
def timeworked(request):
	...
	example call: **whatever_host**/timeworked/?start_time=08:00:00&end_time=17:00:00
	should return a json looks like:
	{'business_seconds': 32400}
	"""

	if weekdayCheck == 'Saturday': # 'Saturday':
		return Response({
		"Please note, No calculations allowed on Weekends."
			})
		...
		return Response({
		"Today is a public holiday in RSA. Please note, no calculations allowed on SA public holidays."
			})
		...
		try:
			...
			if numberSPulled[0] > 24 or numberSPulled[1] > 60 or numberSPulled[2] > 60:
				return Response({
				"Please make sure that hours are less than 24, minutes less than 60 and Seconds less than 60."
				})
			...
			if start_time < 28800:
				return Response({
				"Please note: No public holidays. Weekdays only, from 08:00 - 17:00."
				})
			...
			if end_time > start_time:
				...
				return Response({
				"The End time cannot be less than the Start time. Please review and try again."
				})
			
			return Response({
				"business_seconds": timeDiff
				})
